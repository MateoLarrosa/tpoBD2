    ...
    public void registrarMedicion(Sensor sensor, String tipo, int anio, int mes, String fecha, double valor) {
        ...
        if (fecha != null && !fecha.isBlank()) {
            try {
                ...
                fechaDate = java.util.Date.from(localDate.atStartOfDay(java.time.ZoneOffset.UTC).toInstant());
            } catch (Exception e) {
        ...
        MedicionesPorZona mz = (zona == null || zona.isBlank()) ? null
                : new MedicionesPorZona(sensorId, tipo, zona, anio, mes, fechaDate, valor);

        MedicionesPorPais mp = (pais == null || pais.isBlank()) ? null
                : new MedicionesPorPais(sensorId, tipo, pais, anio, mes, fechaDate, valor);

        MedicionesPorCiudad mc = (ciudad == null || ciudad.isBlank()) ? null
                : new MedicionesPorCiudad(sensorId, tipo, ciudad, anio, mes, fechaDate, valor);
